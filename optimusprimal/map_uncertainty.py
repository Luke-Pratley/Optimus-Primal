...
def create_local_credible_interval(x_sol, region_size, function, bound, iters,
                                   tol, bottom, top):
    """Bisection method for finding credible interval."""

    region = np.zeros(x_sol.shape)
    logger.info("Calculating credible interval for %s superpxiels.",
                region.shape)
    if len(x_sol.shape) > 1:
        ...
        for i in range(dsizey):
            for j in range(dsizex):
                mask = np.roll(np.roll(region, shift=i * region_size, axis=0),
                               shift=j * region_size,
                               axis=1)
                ...
                def obj(eta):
                    return function(x_sol * (1. - mask) + eta * mask) - bound
                ...
                logger.info(
                    ...
                    i, j, error_m[i, j], error_p[i, j], x_sum)
    else:
        ...
        for i in range(dsizey):
            ...
            logger.info(
                "[Credible Interval] %s has interval (%s, %s) with sum %s", i,
                error_m[i], error_p[i], x_sum)
    return error_p, error_m, mean


def create_local_credible_interval_fast(x_sol, phi, psi, region_size, function,
                                        bound, iters, tol, bottom, top):
    """Bisection method for finding credible interval."""
    ...
    if len(x_sol.shape) > 1:
        ...
        for i in range(dsizey):
            for j in range(dsizex):
                mask = mask * 0
                mask[(i * region_size):((i + 1) * region_size),
                     (j * region_size):((j + 1) * region_size)] = 1
                ...
                def obj(eta):
                    return function(data_sol, eta * data_mask, wav_sol,
                                    wav_mask * eta) - bound
